...
export default async function ProductDetailsPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  try {
    ...
    const ACCESSORY_MAP: Array<[RegExp, string[]]> = [
      ...
      [/камер|видеонаблюд|тепловизион/i, ["Кронштейны и аксессуары#кронштейн", "Жёсткие диски", "СКС (витая пара)"]],
      [/коммутатор|маршрутизатор/i, ["SFP-модули и трансиверы", "СКС (витая пара)", "Телекоммуникационные шкафы"]],
      [/wi-?fi|радиомост|беспровод|усилител/i, ["СКС (витая пара)", "Маршрутизаторы", "Кронштейны и аксессуары#кронштейн"]],
      ...
      [/скуд|турникет|терминал|считыват|замок|доступ|металлодетект/i, ["Кронштейны и аксессуары#уголок", "ИБП и электропитание", "Кабель"]],
      ...
      [/скс|витая пара/i, ["Коммутаторы", "Инструменты#обжим", "Телекоммуникационные шкафы"]],
      [/кабель/i, ["Приборы и модули", "Извещатели", "Оповещение"]],
      [/шкаф/i, ["ИБП и электропитание", "Коммутаторы", "Кронштейны и аксессуары#пилот"]],
      ...
      [/дисплеи|монитор/i, ["IP-видеорегистраторы (NVR)", "Кронштейны и аксессуары#кронштейн", "Кабель"]],
    ...
    if (categoryInfo) {
      ...
      if (accCats.length) {
        try {
          // Релевантность вместо «первых по алфавиту»: ценовой коридор 3–70% от цены
          // товара уходит В САМ запрос (иначе лимит выборки отрезает кириллические
          // названия, стоящие в алфавите после латиницы), «Тип#слово» — точечный
          // q-фильтр внутри типа (кронштейны без 4G-модемов). Пустой коридор —
          // повторный запрос без цен. Плюс бренд-бонус и ротация от id товара.
          const prodUzs = (() => {
