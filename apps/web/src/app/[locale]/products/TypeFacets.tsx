...
const VISIBLE = 5; // сколько значений показываем до «Ещё N»
...
export function TypeFacets({ facets }: { facets: ProductFacets }) {
  ...
  const hasActive = brands.length > 0 || Object.keys(chars).length > 0 || !!sp.get("priceMin") || !!sp.get("priceMax");

  return (
    <aside className="space-y-1.5 text-sm">
      <style>{`
      ...
      {facets.chars.map((c) => (
        <Group key={c.key} title={c.key} icon={<IconSpec />}>
          <ValueList
            items={c.values}
            render={(v) => {
