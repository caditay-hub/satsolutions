#!/usr/bin/env bash
# Деплой satsolutions: GitHub(main) -> сервер. Канонная версия в репозитории;
# на сервере исполняется копия /root/deploy-satweb.sh (держать в синхроне).
set -euo pipefail
cd /var/www/satweb
...
#    и фронт-правки видны не сразу. Удаляем prerender → первый запрос рендерит из нового page.js.
WEB_APP=/var/www/satweb/apps/web/.next/server/app
if [ -d "$WEB_APP" ]; then
  find "$WEB_APP" -type f \( -name 'catalog.html' -o -name 'catalog.rsc' -o -name 'catalog.meta' \) -delete 2>/dev/null || true
  find "$WEB_APP" -path '*/catalog/other*' -type f -delete 2>/dev/null || true
  sleep 2
